...
class TestAccountListCreate:
    ...
    def test_list_accounts_with_status_filter(self):
        ...
        response = self.client.get(
            "/accounts/", {"status": AccountStatus.ACTIVE}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) == 1
    ...
    def test_list_accounts_with_search(self):
        ...
        response = self.client.get("/accounts/", {"search": "Acme"}, format="json")
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["results"]) == 1
...
class TestAccountRetrieve:
    ...
    def setup_method(self):
        ...
        self.client = APIClient()
        self.user = UserModel.objects.create_user(
            username="testuser_retrieve", password="testpass123", is_staff=True
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_user_can_retrieve_own_account(self):
        ...
        regular_user = UserModel.objects.create_user(
            username="regular3", password="testpass123", is_staff=False
        )
        account = Account.objects.create(
            name="My Account",
    ...
    def test_user_can_retrieve_other_users_account(self):
        ...
        other_user = UserModel.objects.create_user(
            username="other", password="testpass123", is_staff=False
        )
        account = Account.objects.create(
            name="Owner Account",
            owner_user=owner,
...
class TestAccountUpdate:
    ...
    def setup_method(self):
        ...
        self.client = APIClient()
        self.user = UserModel.objects.create_user(
            username="testuser_update", password="testpass123", is_staff=True
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_update_account_returns_200(self):
        ...
        response = self.client.put(f"/accounts/{account.id}/", payload, format="json")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_account_with_duplicate_account_number_fails(self):
        ...
        response = self.client.put(f"/accounts/{account1.id}/", payload, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "account_number" in response.data
    ...
    def test_non_admin_owner_can_update_own_account(self):
        """Test non-admin owner can update their own account."""
        ...
        # Authenticate as non-admin owner
        client = APIClient()
        client.force_authenticate(user=regular_user)
    ...
    def test_user_cannot_update_other_users_account(self):
        ...
        owner = UserModel.objects.create_user(
            username="owner2", password="testpass123", is_staff=False
        )
        other_user = UserModel.objects.create_user(
            username="other2", password="testpass123", is_staff=False
        )
        account = Account.objects.create(
            name="Owner Account",
            owner_user=owner,
...
class TestAccountDelete:
    ...
    def setup_method(self):
        ...
        self.client = APIClient()
        self.user = UserModel.objects.create_user(
            username="testuser_delete", password="testpass123", is_staff=True
        )
        self.client.force_authenticate(user=self.user)
    ...
    def test_non_admin_owner_can_delete_own_account(self):
        """Test non-admin owner can delete their own account."""
        # Create a non-admin user
        regular_user = UserModel.objects.create_user(
            username="regular2", password="testpass123", is_staff=False
        )
        account = Account.objects.create(
            name="Test Account",
            owner_user=regular_user,
