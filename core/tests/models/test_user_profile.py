...
class TestUserProfileCreation:
    ...
    def test_create_profile_linked_to_user(self):
        ...
        user = user_model.objects.create_user(username="profileuser", password="pass")
        assert user.profile.user == user

    def test_role_defaults_to_none(self):
        """role defaults to None when not specified."""
        user = user_model.objects.create_user(username="noroleuser", password="pass")
    ...
    def test_role_accepts_all_valid_values(self, role):
        ...
        user = user_model.objects.create_user(username=f"{role}user", password="pass")
    ...
    def test_str_representation(self):
        ...
        user = user_model.objects.create_user(username="struser", password="pass")
...
class TestUserProfileSignal:
    ...
    def test_profile_auto_created_on_user_save(self):
        ...
        user = user_model.objects.create_user(username="signaluser", password="pass")
        assert UserProfile.objects.filter(user=user).exists()

    def test_signal_does_not_fire_on_user_update(self):
        """Updating a User does not create a duplicate profile."""
        user = user_model.objects.create_user(username="updateuser", password="pass")
    ...
    def test_get_or_create_prevents_duplicate_profiles(self):
        ...
        user = user_model.objects.create_user(
            username="idempotentuser", password="pass"
        )
        create_user_profile(sender=user_model, instance=user, created=True)
        assert UserProfile.objects.filter(user=user).count() == 1
